...
export async function main(ns) {
	...
	for (let i = 0; i < everyserver.length; i++) {
		if (ns.fileExists('brutessh.exe')) {
			ns.brutessh(everyserver[i]);
		}
		if (ns.fileExists('ftpcrack.exe')) {
			ns.ftpcrack(everyserver[i]);
		}
		if (ns.fileExists('relaysmtp.exe')) {
			ns.relaysmtp(everyserver[i]);
		}
		if (ns.fileExists('httpwork.exe')) {
			ns.httpworm(everyserver[i]);
		}
		if (ns.fileExists('sqlinject.exe')) {
			ns.sqlinject(everyserver[i]);
		}
		try{
		ns.nuke(everyserver[i]);
		}
		catch(error){
			continue;
		}
